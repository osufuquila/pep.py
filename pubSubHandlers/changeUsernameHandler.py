...
def handleUsernameChange(userID, newUsername, targetToken=None):
    try:
        ...
        if targetToken is not None:
            targetToken.kick(
                "Your username has been changed to {}. Please log in again.".format(
                    newUsername,
                ),
                "username_change",
            )
...
class handler(generalPubSubHandler.generalPubSubHandler):
    ...
    def handle(self, data):
        ...
        if targetToken is None:
            ...
            if targetToken.irc or (
                targetToken.actionID != actions.PLAYING
                and targetToken.actionID != actions.MULTIPLAYING
            ):
                ...
                glob.redis.set(
                    "ripple:change_username_pending:{}".format(data["userID"]),
                    data["newUsername"],
                )
